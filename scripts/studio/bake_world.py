...
def bake():
    ...
    visible_by_view={}
    faces_by_view={}
    for view in sorted({obj.get("view","artwork") for obj in objects}-{ "swatch" }):
        ...
        for triangle, owner, face_id in zip(triangles, owners, face_ids):
            if objects[int(owner)].get("view","artwork")!=view:
                continue
        ...
        visible_by_view[view]=visible
        faces_by_view[view]=visible_face
    ...
    for index,page in enumerate(pages):
        Image.fromarray(page).save(OUT/f"world-atlas-{index}.png",optimize=True)
